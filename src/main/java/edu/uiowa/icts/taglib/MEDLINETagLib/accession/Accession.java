...
import edu.uiowa.icts.taglib.MEDLINETagLib.dataBank.DataBank;
...
public class Accession extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theAccessionIterator == null && theDataBank == null && accnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select accession from medline10.accession where pmid = ? and seqnum = ? and accnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline10.accession set accession = ? where pmid = ? and seqnum = ? and accnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (accession == null)
				accession = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline10.accession(pmid,seqnum,accnum,accession) values (?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
			stmt.setInt(3,accnum);
