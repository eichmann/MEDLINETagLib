...
public class SpaceflightMissionIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String spaceflightMissionCountByArticle(String pmid) throws JspTagException {
		...
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline10.spaceflight_mission where 1=1"
						+ " and pmid = ?"
						);
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline10.spaceflight_mission.pmid, medline10.spaceflight_mission.seqnum from " + generateFromClause() + " where 1=1"
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline10.spaceflight_mission");
