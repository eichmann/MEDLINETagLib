...
//			System.out.println("pmid: " + pmid);
	...
	static void emit(int pmid) throws Exception {
		PreparedStatement artStmt = medConn.prepareStatement("select title,medline_pgn from medline11.article where pmid=?");
		...
		PreparedStatement jourStmt = medConn.prepareStatement("select iso_abbreviation,volume,issue,pub_year,medline_date from medline11.journal where pmid=?");
		...
		PreparedStatement authStmt = medConn.prepareStatement("select last_name,fore_name,initials,suffix,collective_name from medline11.author where pmid=? order by seqnum");
