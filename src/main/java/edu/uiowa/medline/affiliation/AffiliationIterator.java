...
public class AffiliationIterator extends MEDLINETagLibBodyTagSupport {
    int pmid = 0;
    int seqnum = 0;
    int anum = 0;
    String label = null;
	...
	public static String affiliationCountByAuthor(String pmid, String seqnum) throws JspTagException {
		int count = 0;
		AffiliationIterator theIterator = new AffiliationIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline15.affiliation where 1=1"
						+ " and pmid = ?"
						+ " and seqnum = ?"
						);
	...
	public static Boolean authorHasAffiliation(String pmid, String seqnum) throws JspTagException {
		return ! affiliationCountByAuthor(pmid, seqnum).equals("0");
	}

	public static Boolean affiliationExists (String pmid, String seqnum, String anum) throws JspTagException {
		int count = 0;
		AffiliationIterator theIterator = new AffiliationIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline15.affiliation where 1=1"
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline15.affiliation.pmid, medline15.affiliation.seqnum, medline15.affiliation.anum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
                                                        + (seqnum == 0 ? "" : " and seqnum = ?")
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline15.affiliation");
