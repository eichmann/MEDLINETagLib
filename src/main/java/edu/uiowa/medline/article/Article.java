...
public class Article extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theArticleIterator == null && pmid == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select date_created,date_completed,date_revised,title,start_page,end_page,medline_pgn,copyright,vernacular_title,country,ta,nlm_unique_id,issn_linking,reference_count,pub_model,status from medline18.article where pmid = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline18.article set date_created = ?, date_completed = ?, date_revised = ?, title = ?, start_page = ?, end_page = ?, medline_pgn = ?, copyright = ?, vernacular_title = ?, country = ?, ta = ?, nlm_unique_id = ?, issn_linking = ?, reference_count = ?, pub_model = ?, status = ? where pmid = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (status == null)
				status = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline18.article(pmid,date_created,date_completed,date_revised,title,start_page,end_page,medline_pgn,copyright,vernacular_title,country,ta,nlm_unique_id,issn_linking,reference_count,pub_model,status) values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)");
			stmt.setInt(1,pmid);
