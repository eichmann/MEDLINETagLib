...
public class ArticleIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String articleCount() throws JspTagException {
		...
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline14.article"
						);
			...
			log.error("JDBC error generating Article iterator", e);
	...
	public static Boolean articleExists (String pmid) throws JspTagException {
		...
		ArticleIterator theIterator = new ArticleIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline14.article where 1=1"
						+ " and pmid = ?"
						);
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline14.article.pmid from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
            ...
            log.error("JDBC error generating Article iterator: " + stat.toString(), e);
            clearServiceState();
            freeConnection();
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline14.article");
    ...
    public int doAfterBody() throws JspTagException {
        try {
            ...
            log.error("JDBC error iterating across Article", e);
            clearServiceState();
            freeConnection();
    ...
    public int doEndTag() throws JspTagException, JspException {
        try {
            ...
            log.error("JDBC error ending Article iterator",e);
