...
public class ArticleDate extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theArticleDateIterator == null && theArticle == null && seqnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select year,month,day,type from medline14.article_date where pmid = ? and seqnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline14.article_date set year = ?, month = ?, day = ?, type = ? where pmid = ? and seqnum = ?");
				stmt.setInt(1,year);
				stmt.setInt(2,month);
				stmt.setInt(3,day);
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (type == null)
				type = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline14.article_date(pmid,seqnum,year,month,day,type) values (?,?,?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
			stmt.setInt(3,year);
			stmt.setInt(4,month);
