...
public class ArticleDate extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theArticleDateIterator == null && theArticle == null && seqnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select year,month,day from medline10.article_date where pmid = ? and seqnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline10.article_date set year = ?, month = ?, day = ? where pmid = ? and seqnum = ?");
				stmt.setInt(1,year);
				stmt.setInt(2,month);
				stmt.setInt(3,day);
				stmt.setInt(4,pmid);
	...
	public void insertEntity() throws JspException {
		try {
			...
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline10.article_date(pmid,seqnum,year,month,day) values (?,?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
			stmt.setInt(3,year);
			stmt.setInt(4,month);
