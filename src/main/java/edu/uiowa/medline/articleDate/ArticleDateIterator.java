...
public class ArticleDateIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String articleDateCountByArticle(String pmid) throws JspTagException {
		...
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline13.article_date where 1=1"
						+ " and pmid = ?"
						);
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline13.article_date.pmid, medline13.article_date.seqnum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline13.article_date");
