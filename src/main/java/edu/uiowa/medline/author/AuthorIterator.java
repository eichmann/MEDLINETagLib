...
public class AuthorIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String authorCountByArticle(String pmid) throws JspTagException {
		int count = 0;
		AuthorIterator theIterator = new AuthorIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline12.author where 1=1"
						+ " and pmid = ?"
						);
	...
	public static Boolean articleHasAuthor(String pmid) throws JspTagException {
		return ! authorCountByArticle(pmid).equals("0");
	}

	public static Boolean authorExists (String pmid, String seqnum) throws JspTagException {
		int count = 0;
		AuthorIterator theIterator = new AuthorIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline12.author where 1=1"
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline12.author.pmid, medline12.author.seqnum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline12.author");
