...
public class AuthorAffiliationDeleter extends MEDLINETagLibBodyTagSupport {
    int pmid = 0;
    int seqnum = 0;
    int anum = 0;
    String label = null;
	...
	private static final Log log = LogFactory.getLog(AuthorAffiliationDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from medline16.author_affiliation where 1=1"
                                                        + (pmid == 0 ? "" : " and pmid = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? ")
                                                        + (anum == 0 ? "" : " and anum = ? "));
            ...
            log.error("JDBC error generating AuthorAffiliation deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating AuthorAffiliation deleter");
        } finally {
            freeConnection();
        }
