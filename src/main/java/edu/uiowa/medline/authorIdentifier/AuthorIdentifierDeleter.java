...
public class AuthorIdentifierDeleter extends MEDLINETagLibBodyTagSupport {
    int pmid = 0;
    int seqnum = 0;
    int inum = 0;
    String source = null;
	...
	private static final Log log = LogFactory.getLog(AuthorIdentifierDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from medline18.author_identifier where 1=1"
                                                        + (pmid == 0 ? "" : " and pmid = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? ")
                                                        + (inum == 0 ? "" : " and inum = ? "));
            ...
            log.error("JDBC error generating AuthorIdentifier deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating AuthorIdentifier deleter");
        } finally {
            freeConnection();
        }
