...
public class CitationSubset extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theCitationSubsetIterator == null && theArticle == null && seqnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select label from medline18.citation_subset where pmid = ? and seqnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline18.citation_subset set label = ? where pmid = ? and seqnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (label == null)
				label = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline18.citation_subset(pmid,seqnum,label) values (?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
