...
public class CitationSubsetIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String citationSubsetCountByArticle(String pmid) throws JspTagException {
		int count = 0;
		CitationSubsetIterator theIterator = new CitationSubsetIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline16.citation_subset where 1=1"
						+ " and pmid = ?"
						);
	...
	public static Boolean articleHasCitationSubset(String pmid) throws JspTagException {
		return ! citationSubsetCountByArticle(pmid).equals("0");
	}

	public static Boolean citationSubsetExists (String pmid, String seqnum) throws JspTagException {
		int count = 0;
		CitationSubsetIterator theIterator = new CitationSubsetIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline16.citation_subset where 1=1"
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline16.citation_subset.pmid, medline16.citation_subset.seqnum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline16.citation_subset");
