...
public class ClusterPattern extends MEDLINETagLibTagSupport {

	static ClusterPattern currentInstance = null;
	boolean commitNeeded = false;
	boolean newRecord = false;

	private static final Log log = LogFactory.getLog(ClusterPattern.class);
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			ClusterPatternIterator theClusterPatternIterator = (ClusterPatternIterator)findAncestorWithClass(this, ClusterPatternIterator.class);

			if (theClusterPatternIterator != null) {
				cid = theClusterPatternIterator.getCid();
				seqnum = theClusterPatternIterator.getSeqnum();
			}

			if (theClusterPatternIterator == null && theDocumentCluster == null && seqnum == 0) {
				// no seqnum was provided - the default is to assume that it is a new ClusterPattern and to generate a new seqnum
				seqnum = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or seqnum was provided as an attribute - we need to load a ClusterPattern from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select last_name,fore_name,occurrences from medline_clustering.cluster_pattern where cid = ? and seqnum = ?");
				stmt.setInt(1,cid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline_clustering.cluster_pattern set last_name = ?, fore_name = ?, occurrences = ? where cid = ? and seqnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			if (seqnum == 0) {
				seqnum = Sequence.generateID();
				log.debug("generating new ClusterPattern " + seqnum);
			}
			...
			if (foreName == null)
				foreName = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline_clustering.cluster_pattern(cid,seqnum,last_name,fore_name,occurrences) values (?,?,?,?,?)");
			stmt.setInt(1,cid);
			stmt.setInt(2,seqnum);
