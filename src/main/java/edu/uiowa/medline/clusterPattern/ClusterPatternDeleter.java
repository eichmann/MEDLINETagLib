...
public class ClusterPatternDeleter extends MEDLINETagLibBodyTagSupport {
    int cid = 0;
    int seqnum = 0;
	...
	private static final Log log = LogFactory.getLog(ClusterPatternDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from medline_clustering.cluster_pattern where 1=1"
                                                        + (cid == 0 ? "" : " and cid = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? "));
            ...
            log.error("JDBC error generating ClusterPattern deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating ClusterPattern deleter");
        } finally {
            freeConnection();
        }
