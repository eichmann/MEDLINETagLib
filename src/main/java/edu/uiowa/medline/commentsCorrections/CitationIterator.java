...
public class CitationIterator extends MEDLINETagLibBodyTagSupport {
    int pmid = 0;
    int seqnum = 0;
    String refType = null;
    String source = null;
	...
	public static Boolean articleHasCitedArticles(String pmid) throws JspTagException {
		return ! citationCount(pmid).equals("0");
	}
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline16.comments_corrections.ref_pmid, medline16.comments_corrections.seqnum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
