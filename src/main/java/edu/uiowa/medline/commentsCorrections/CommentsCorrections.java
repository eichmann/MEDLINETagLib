...
public class CommentsCorrections extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theCommentsCorrectionsIterator == null && theArticle == null && seqnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select ref_type,source,ref_pmid,note from medline11.comments_corrections where pmid = ? and seqnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline11.comments_corrections set ref_type = ?, source = ?, ref_pmid = ?, note = ? where pmid = ? and seqnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (note == null)
				note = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline11.comments_corrections(pmid,seqnum,ref_type,source,ref_pmid,note) values (?,?,?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
