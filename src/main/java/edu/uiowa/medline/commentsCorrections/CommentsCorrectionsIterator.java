...
public class CommentsCorrectionsIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String commentsCorrectionsCountByArticle(String pmid) throws JspTagException {
		int count = 0;
		CommentsCorrectionsIterator theIterator = new CommentsCorrectionsIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline15.comments_corrections where 1=1"
						+ " and pmid = ?"
						);
	...
	public static Boolean articleHasCommentsCorrections(String pmid) throws JspTagException {
		return ! commentsCorrectionsCountByArticle(pmid).equals("0");
	}

	public static Boolean commentsCorrectionsExists (String pmid, String seqnum) throws JspTagException {
		int count = 0;
		CommentsCorrectionsIterator theIterator = new CommentsCorrectionsIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline15.comments_corrections where 1=1"
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline15.comments_corrections.pmid, medline15.comments_corrections.seqnum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline15.comments_corrections");
