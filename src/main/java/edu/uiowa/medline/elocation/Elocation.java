...
public class Elocation extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theElocationIterator == null && theArticle == null && seqnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select eid,type from medline18.elocation where pmid = ? and seqnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline18.elocation set eid = ?, type = ? where pmid = ? and seqnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (type == null)
				type = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline18.elocation(pmid,seqnum,eid,type) values (?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
