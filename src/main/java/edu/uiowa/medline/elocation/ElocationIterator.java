...
public class ElocationIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String elocationCountByArticle(String pmid) throws JspTagException {
		int count = 0;
		ElocationIterator theIterator = new ElocationIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline14.elocation where 1=1"
						+ " and pmid = ?"
						);
	...
	public static Boolean articleHasElocation(String pmid) throws JspTagException {
		return ! elocationCountByArticle(pmid).equals("0");
	}

	public static Boolean elocationExists (String pmid, String seqnum) throws JspTagException {
		int count = 0;
		ElocationIterator theIterator = new ElocationIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline14.elocation where 1=1"
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline14.elocation.pmid, medline14.elocation.seqnum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline14.elocation");
