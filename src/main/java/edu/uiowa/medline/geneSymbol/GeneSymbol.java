...
public class GeneSymbol extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theGeneSymbolIterator == null && theArticle == null && seqnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select symbol from medline16.gene_symbol where pmid = ? and seqnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline16.gene_symbol set symbol = ? where pmid = ? and seqnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (symbol == null)
				symbol = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline16.gene_symbol(pmid,seqnum,symbol) values (?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
