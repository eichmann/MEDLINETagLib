...
public class GeneralNoteIterator extends MEDLINETagLibBodyTagSupport {
	...
	private static final Log log = LogFactory.getLog(GeneralNoteIterator.class);
	...
	public static String generalNoteCountByArticle(String pmid) throws JspTagException {
		...
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline14.general_note where 1=1"
						+ " and pmid = ?"
						);
			...
			log.error("JDBC error generating GeneralNote iterator", e);
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline14.general_note.pmid, medline14.general_note.seqnum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
            ...
            log.error("JDBC error generating GeneralNote iterator: " + stat.toString(), e);
            clearServiceState();
            freeConnection();
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline14.general_note");
    ...
    public int doAfterBody() throws JspTagException {
        try {
            ...
            log.error("JDBC error iterating across GeneralNote", e);
            clearServiceState();
            freeConnection();
    ...
    public int doEndTag() throws JspTagException, JspException {
        try {
            ...
            log.error("JDBC error ending GeneralNote iterator",e);
