...
public class GeneralNoteIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String generalNoteCountByArticle(String pmid) throws JspTagException {
		...
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline10.general_note where 1=1"
						+ " and pmid = ?"
						);
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline10.general_note.pmid, medline10.general_note.seqnum from " + generateFromClause() + " where 1=1"
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline10.general_note");
