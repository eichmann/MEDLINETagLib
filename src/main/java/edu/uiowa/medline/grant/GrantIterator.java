...
public class GrantIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String grantCountByArticle(String pmid) throws JspTagException {
		int count = 0;
		GrantIterator theIterator = new GrantIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline16.grant where 1=1"
						+ " and pmid = ?"
						);
	...
	public static Boolean articleHasGrant(String pmid) throws JspTagException {
		return ! grantCountByArticle(pmid).equals("0");
	}

	public static Boolean grantExists (String pmid, String seqnum) throws JspTagException {
		int count = 0;
		GrantIterator theIterator = new GrantIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline16.grant where 1=1"
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline16.grant.pmid, medline16.grant.seqnum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline16.grant");
