...
public class InvestigatorAffiliationIdentifier extends MEDLINETagLibTagSupport {
	...
	static InvestigatorAffiliationIdentifier currentInstance = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			InvestigatorAffiliationIdentifierIterator theInvestigatorAffiliationIdentifierIterator = (InvestigatorAffiliationIdentifierIterator)findAncestorWithClass(this, InvestigatorAffiliationIdentifierIterator.class);

			if (theInvestigatorAffiliationIdentifierIterator != null) {
				pmid = theInvestigatorAffiliationIdentifierIterator.getPmid();
				seqnum = theInvestigatorAffiliationIdentifierIterator.getSeqnum();
				anum = theInvestigatorAffiliationIdentifierIterator.getAnum();
				inum = theInvestigatorAffiliationIdentifierIterator.getInum();
			}

			if (theInvestigatorAffiliationIdentifierIterator == null && theInvestigatorAffiliation == null && inum == 0) {
				// no inum was provided - the default is to assume that it is a new InvestigatorAffiliationIdentifier and to generate a new inum
				inum = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or inum was provided as an attribute - we need to load a InvestigatorAffiliationIdentifier from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select source,identifier from medline16.investigator_affiliation_identifier where pmid = ? and seqnum = ? and anum = ? and inum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline16.investigator_affiliation_identifier set source = ?, identifier = ? where pmid = ? and seqnum = ? and anum = ? and inum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			if (inum == 0) {
				inum = Sequence.generateID();
				log.debug("generating new InvestigatorAffiliationIdentifier " + inum);
			}
			...
			if (identifier == null)
				identifier = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline16.investigator_affiliation_identifier(pmid,seqnum,anum,inum,source,identifier) values (?,?,?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
			stmt.setInt(3,anum);
			stmt.setInt(4,inum);
