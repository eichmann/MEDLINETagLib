...
public class InvestigatorAffiliationIdentifier extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theInvestigatorAffiliationIdentifierIterator == null && theInvestigatorAffiliation == null && inum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select source,identifier from medline18.investigator_affiliation_identifier where pmid = ? and seqnum = ? and anum = ? and inum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline18.investigator_affiliation_identifier set source = ?, identifier = ? where pmid = ? and seqnum = ? and anum = ? and inum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (identifier == null)
				identifier = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline18.investigator_affiliation_identifier(pmid,seqnum,anum,inum,source,identifier) values (?,?,?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
			stmt.setInt(3,anum);
			stmt.setInt(4,inum);
