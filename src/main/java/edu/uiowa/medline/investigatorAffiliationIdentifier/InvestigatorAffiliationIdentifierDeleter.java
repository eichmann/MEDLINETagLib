...
public class InvestigatorAffiliationIdentifierDeleter extends MEDLINETagLibBodyTagSupport {
    int pmid = 0;
    int seqnum = 0;
    int anum = 0;
    int inum = 0;
	...
	private static final Log log = LogFactory.getLog(InvestigatorAffiliationIdentifierDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from medline16.investigator_affiliation_identifier where 1=1"
                                                        + (pmid == 0 ? "" : " and pmid = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? ")
                                                        + (anum == 0 ? "" : " and anum = ? ")
                                                        + (inum == 0 ? "" : " and inum = ? "));
            ...
            log.error("JDBC error generating InvestigatorAffiliationIdentifier deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating InvestigatorAffiliationIdentifier deleter");
        } finally {
            freeConnection();
        }
