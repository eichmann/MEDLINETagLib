...
public class Journal extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theJournalIterator == null && theArticle == null && pmid == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select issn,volume,issue,pub_year,pub_month,pub_day,pub_season,medline_date,title,iso_abbreviation from medline18.journal where pmid = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline18.journal set issn = ?, volume = ?, issue = ?, pub_year = ?, pub_month = ?, pub_day = ?, pub_season = ?, medline_date = ?, title = ?, iso_abbreviation = ? where pmid = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (isoAbbreviation == null)
				isoAbbreviation = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline18.journal(pmid,issn,volume,issue,pub_year,pub_month,pub_day,pub_season,medline_date,title,iso_abbreviation) values (?,?,?,?,?,?,?,?,?,?,?)");
			stmt.setInt(1,pmid);
