...
public class JournalIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String journalCountByArticle(String pmid) throws JspTagException {
		...
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline15.journal where 1=1"
						+ " and pmid = ?"
						);
			...
			log.error("JDBC error generating Journal iterator", e);
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline15.journal.pmid from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
            ...
            log.error("JDBC error generating Journal iterator: " + stat.toString(), e);
            clearServiceState();
            freeConnection();
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline15.journal");
    ...
    public int doAfterBody() throws JspTagException {
        try {
            ...
            log.error("JDBC error iterating across Journal", e);
            clearServiceState();
            freeConnection();
    ...
    public int doEndTag() throws JspTagException, JspException {
        try {
            ...
            log.error("JDBC error ending Journal iterator",e);
