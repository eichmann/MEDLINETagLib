...
public class JournalIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String journalCountByArticle(String pmid) throws JspTagException {
		...
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline10.journal where 1=1"
						+ " and pmid = ?"
						);
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline10.journal.pmid from " + generateFromClause() + " where 1=1"
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline10.journal");
