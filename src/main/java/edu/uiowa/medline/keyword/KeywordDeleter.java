...
public class KeywordDeleter extends MEDLINETagLibBodyTagSupport {
    int pmid = 0;
    int seqnum = 0;
    String keyword = null;
    boolean major = false;
	...
	private static final Log log = LogFactory.getLog(KeywordDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from medline14.keyword where 1=1"
                                                        + (pmid == 0 ? "" : " and pmid = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? "));
            ...
            log.error("JDBC error generating Keyword deleter", e);
            clearServiceState();
