...
public class KeywordIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String keywordCountByArticle(String pmid) throws JspTagException {
		int count = 0;
		KeywordIterator theIterator = new KeywordIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline13.keyword where 1=1"
						+ " and pmid = ?"
						);
	...
	public static Boolean articleHasKeyword(String pmid) throws JspTagException {
		return ! keywordCountByArticle(pmid).equals("0");
	}

	public static Boolean keywordExists (String pmid, String seqnum) throws JspTagException {
		int count = 0;
		KeywordIterator theIterator = new KeywordIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline13.keyword where 1=1"
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline13.keyword.pmid, medline13.keyword.seqnum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline13.keyword");
