...
public class Language extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theLanguageIterator == null && theArticle == null && seqnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select language from medline13.language where pmid = ? and seqnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline13.language set language = ? where pmid = ? and seqnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (language == null)
				language = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline13.language(pmid,seqnum,language) values (?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
