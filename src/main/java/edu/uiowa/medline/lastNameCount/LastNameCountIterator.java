...
public class LastNameCountIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String lastNameCountCount() throws JspTagException {
		...
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline15.last_name_count"
						);
	...
	public static Boolean lastNameCountExists (String lastName) throws JspTagException {
		...
		LastNameCountIterator theIterator = new LastNameCountIterator();
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline15.last_name_count where 1=1"
						+ " and last_name = ?"
						);
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline15.last_name_count.last_name from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline15.last_name_count");
