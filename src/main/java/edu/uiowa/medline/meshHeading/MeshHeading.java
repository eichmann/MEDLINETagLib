...
public class MeshHeading extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theMeshHeadingIterator == null && theArticle == null && seqnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select descriptor_name,major,type,id from medline18.mesh_heading where pmid = ? and seqnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline18.mesh_heading set descriptor_name = ?, major = ?, type = ?, id = ? where pmid = ? and seqnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (ID == null)
				ID = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline18.mesh_heading(pmid,seqnum,descriptor_name,major,type,id) values (?,?,?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
