...
public class MeshHeadingDeleter extends MEDLINETagLibBodyTagSupport {
    int pmid = 0;
    int seqnum = 0;
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from medline18.mesh_heading where 1=1"
                                                        + (pmid == 0 ? "" : " and pmid = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? "));
            ...
            log.error("JDBC error generating MeshHeading deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating MeshHeading deleter");
        } finally {
            freeConnection();
        }
