...
public class NameId extends MEDLINETagLibTagSupport {

	static NameId currentInstance = null;
	...
	private NameId cachedNameId = null;
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			NameIdIterator theNameIdIterator = (NameIdIterator)findAncestorWithClass(this, NameIdIterator.class);

			if (theNameIdIterator != null) {
				pmid = theNameIdIterator.getPmid();
				seqnum = theNameIdIterator.getSeqnum();
				nnum = theNameIdIterator.getNnum();
			}

			if (theNameIdIterator == null && theAuthor == null && nnum == 0) {
				// no nnum was provided - the default is to assume that it is a new NameId and to generate a new nnum
				nnum = Sequence.generateID();
				insertEntity();
			} else {
				// an iterator or nnum was provided as an attribute - we need to load a NameId from the database
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select name_id,source from medline13.name_id where pmid = ? and seqnum = ? and nnum = ?");
				stmt.setInt(1,pmid);
		...
		NameId currentNameId = (NameId) pageContext.getAttribute("tag_nameId");
		if(currentNameId != null){
			cachedNameId = currentNameId;
		}
		currentNameId = this;
		pageContext.setAttribute((var == null ? "tag_nameId" : var), currentNameId);
	...
	public int doEndTag() throws JspException {
		...
		if(this.cachedNameId != null){
			pageContext.setAttribute((var == null ? "tag_nameId" : var), this.cachedNameId);
		}else{
			pageContext.removeAttribute((var == null ? "tag_nameId" : var));
			this.cachedNameId = null;
		}

		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline13.name_id set name_id = ?, source = ? where pmid = ? and seqnum = ? and nnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			if (nnum == 0) {
				nnum = Sequence.generateID();
				log.debug("generating new NameId " + nnum);
			}
			...
			if (source == null)
				source = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline13.name_id(pmid,seqnum,nnum,name_id,source) values (?,?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
			stmt.setInt(3,nnum);
