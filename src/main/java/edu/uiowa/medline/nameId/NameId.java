...
public class NameId extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theNameIdIterator == null && theAuthor == null && nnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select name_id,source from medline15.name_id where pmid = ? and seqnum = ? and nnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline15.name_id set name_id = ?, source = ? where pmid = ? and seqnum = ? and nnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (source == null)
				source = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline15.name_id(pmid,seqnum,nnum,name_id,source) values (?,?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
			stmt.setInt(3,nnum);
