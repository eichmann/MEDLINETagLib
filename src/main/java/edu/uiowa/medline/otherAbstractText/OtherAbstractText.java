...
public class OtherAbstractText extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (theOtherAbstractTextIterator == null && theOtherAbstract == null && tnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select abstract_text,label,category from medline11.other_abstract_text where pmid = ? and seqnum = ? and tnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline11.other_abstract_text set abstract_text = ?, label = ?, category = ? where pmid = ? and seqnum = ? and tnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (category == null)
				category = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline11.other_abstract_text(pmid,seqnum,tnum,abstract_text,label,category) values (?,?,?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
			stmt.setInt(3,tnum);
