...
public class OtherAbstractTextDeleter extends MEDLINETagLibBodyTagSupport {
    int pmid = 0;
    int seqnum = 0;
    int tnum = 0;
	...
	private static final Log log = LogFactory.getLog(OtherAbstractTextDeleter.class);
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from medline18.other_abstract_text where 1=1"
                                                        + (pmid == 0 ? "" : " and pmid = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? ")
                                                        + (tnum == 0 ? "" : " and tnum = ? "));
            if (pmid != 0) stat.setInt(webapp_keySeq++, pmid);
            if (seqnum != 0) stat.setInt(webapp_keySeq++, seqnum);
            if (tnum != 0) stat.setInt(webapp_keySeq++, tnum);
            stat.execute();

			webapp_keySeq = 1;
        } catch (SQLException e) {
            log.error("JDBC error generating OtherAbstractText deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating OtherAbstractText deleter");
        } finally {
            freeConnection();
        }
