...
public class OtherAbstractTextIterator extends MEDLINETagLibBodyTagSupport {
	...
	public static String otherAbstractTextCountByOtherAbstract(String pmid, String seqnum) throws JspTagException {
		...
		try {
			PreparedStatement stat = theIterator.getConnection().prepareStatement("SELECT count(*) from medline16.other_abstract_text where 1=1"
						+ " and pmid = ?"
						+ " and seqnum = ?"
						);
    ...
    public int doStartTag() throws JspException {
      ...
      try {
            ...
            //run select id query  
            webapp_keySeq = 1;
            stat = getConnection().prepareStatement("SELECT medline16.other_abstract_text.pmid, medline16.other_abstract_text.seqnum, medline16.other_abstract_text.tnum from " + generateFromClause() + " where 1=1"
                                                        + generateJoinCriteria()
                                                        + (pmid == 0 ? "" : " and pmid = ?")
                                                        + (seqnum == 0 ? "" : " and seqnum = ?")
    ...
    private String generateFromClause() {
       StringBuffer theBuffer = new StringBuffer("medline16.other_abstract_text");
