...
public class PersonalNameSubject extends MEDLINETagLibTagSupport {
	...
	public int doStartTag() throws JspException {
		...
		try {
			...
			if (thePersonalNameSubjectIterator == null && theArticle == null && seqnum == 0) {
				...
				boolean found = false;
				PreparedStatement stmt = getConnection().prepareStatement("select last_name,fore_name,initials,suffix from medline11.personal_name_subject where pmid = ? and seqnum = ?");
				stmt.setInt(1,pmid);
	...
	public int doEndTag() throws JspException {
		...
		try {
			if (commitNeeded) {
				PreparedStatement stmt = getConnection().prepareStatement("update medline11.personal_name_subject set last_name = ?, fore_name = ?, initials = ?, suffix = ? where pmid = ? and seqnum = ?");
	...
	public void insertEntity() throws JspException {
		try {
			...
			if (suffix == null)
				suffix = "";
			PreparedStatement stmt = getConnection().prepareStatement("insert into medline11.personal_name_subject(pmid,seqnum,last_name,fore_name,initials,suffix) values (?,?,?,?,?,?)");
			stmt.setInt(1,pmid);
			stmt.setInt(2,seqnum);
