...
public class PublicationTypeDeleter extends MEDLINETagLibBodyTagSupport {
    int pmid = 0;
    int seqnum = 0;
    String label = null;
    ...
    public int doStartTag() throws JspException {
        ...
        try {
            int webapp_keySeq = 1;
            stat = getConnection().prepareStatement("DELETE from medline12.publication_type where 1=1"
                                                        + (pmid == 0 ? "" : " and pmid = ? ")
                                                        + (seqnum == 0 ? "" : " and seqnum = ? "));
            ...
            log.error("JDBC error generating PublicationType deleter", e);
            clearServiceState();
            throw new JspTagException("Error: JDBC error generating PublicationType deleter");
        } finally {
            freeConnection();
        }
